<!-- Слайдер -->
<section class="slideshow">
	<a href="#" class="slideshow_left"></a>
	<a href="#" class="slideshow_right"></a>
	<div class="slideshow_wrap">
		<ul class="slideshow_list">
			<li>
				<div class="slideshow_background">
					<img src="<?php echo $pathCommon;?>/img/content/ss_bg.png" alt="">
				</div>
				<div class="slideshow_list_item">
					<div class="slideshow_item_text">
						<div class="slideshow_text_title">
							<img src="<?php echo $pathCommon;?>/img/decor/slogan.png" alt="">
						</div>
						<div class="slideshow_text_area">
							<div class="slideshow_text_sale">
								<span>13,5</span>
							</div>
							<div class="slideshow_text_descript">
								<div class="slideshow_descript_title">Скидки</div>
								<div class="slideshow_descr_for">на электрочайники и кофеварки</div>
							</div>
						</div>
					</div>
					<div class="slideshow_item_images">
						<img src="<?php echo $pathCommon;?>/img/content/teapot.png" alt="">
					</div>
				</div>
				<div class="slideshow_descript">
					<div class="slideshow_descript_inner">
						<div href="#" class="slideshow_descr_title">
							<a href="#">Чайник DL-1255B</a>
						</div>
					<p>корпус из высококачественного жаропрочного стекла</p>
					</div>
				</div>
			</li>
			<li>
				<div class="slideshow_background">
					<img src="<?php echo $pathCommon;?>/img/content/ss_bg.png" alt="">
				</div>
				<div class="slideshow_list_item">
					<div class="slideshow_item_text">
						<div class="slideshow_text_title">
							<img src="<?php echo $pathCommon;?>/img/decor/slogan.png" alt="">
						</div>
						<div class="slideshow_text_area">
							<div class="slideshow_text_sale">
								<span>13,5</span>
							</div>
							<div class="slideshow_text_descript">
								<div class="slideshow_descript_title">Скидки</div>
								<div class="slideshow_descr_for">на электрочайники и кофеварки</div>
							</div>
						</div>
					</div>
					<div class="slideshow_item_images">
						<img src="<?php echo $pathCommon;?>/img/content/teapot2.png" alt="">
					</div>
				</div>
				<div class="slideshow_descript">
					<div class="slideshow_descript_inner">
						<div href="#" class="slideshow_descr_title">
							<a href="#">Чайник DL-1255B</a>
						</div>
						<p>корпус из высококачественного жаропрочного стекла</p>
					</div>
				</div>
			</li>
			<li>
				<div class="slideshow_background">
					<img src="<?php echo $pathCommon;?>/img/content/ss_bg.png" alt="">
				</div>
				<div class="slideshow_list_item">
					<div class="slideshow_item_text">
						<div class="slideshow_text_title">
							<img src="<?php echo $pathCommon;?>/img/decor/slogan.png" alt="">
						</div>
						<div class="slideshow_text_area">
							<div class="slideshow_text_sale">
								<span>13,5</span>
							</div>
							<div class="slideshow_text_descript">
								<div class="slideshow_descript_title">Скидки</div>
								<div class="slideshow_descr_for">на электрочайники и кофеварки</div>
							</div>
						</div>
					</div>
					<div class="slideshow_item_images">
						<img src="<?php echo $pathCommon;?>/img/content/teapot3.png" alt="">
					</div>
				</div>
				<div class="slideshow_descript">
					<div class="slideshow_descript_inner">
						<div href="#" class="slideshow_descr_title">
							<a href="#">Чайник DL-1255B</a>
						</div>
						<p>корпус из высококачественного жаропрочного стекла</p>
					</div>
				</div>
			</li>
			<li>
				<div class="slideshow_background">
					<img src="<?php echo $pathCommon;?>/img/content/ss_bg.png" alt="">
				</div>
				<div class="slideshow_list_item">
					<div class="slideshow_item_text">
						<div class="slideshow_text_title">
							<img src="<?php echo $pathCommon;?>/img/decor/slogan.png" alt="">
						</div>
						<div class="slideshow_text_area">
							<div class="slideshow_text_sale">
								<span>13,5</span>
							</div>
							<div class="slideshow_text_descript">
								<div class="slideshow_descript_title">Скидки</div>
								<div class="slideshow_descr_for">на электрочайники и кофеварки</div>
							</div>
						</div>
					</div>
					<div class="slideshow_item_images">
						<img src="<?php echo $pathCommon;?>/img/content/teapot4.png" alt="">
					</div>
				</div>
				<div class="slideshow_descript">
					<div class="slideshow_descript_inner">
						<div href="#" class="slideshow_descr_title">
							<a href="#">Чайник DL-1255B</a>
						</div>
						<p>корпус из высококачественного жаропрочного стекла</p>
					</div>
				</div>
			</li>
		</ul>
	</div>
</section>